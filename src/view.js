...
const watch = (state, elements, i18nextInstance) => onChange(state, (path, value) => {
  ...
  if (path === 'modal.active') {
    if (value) {
      ...
      modal_years.textContent = i18nextInstance.t(`education.${state.diploms[state.modal.viewedDiplomaId].title}.years`);
      modal_description.textContent = i18nextInstance.t(`education.${state.diploms[state.modal.viewedDiplomaId].title}.description`);
